{ config, lib, pkgs, ... }:
{
  options.programs.neovim.dap.enable = lib.mkEnableOption "Neovim DAP support";

  config = lib.mkIf (config.programs.neovim.enable && config.programs.neovim.dap.enable) {
    programs.neovim = {
